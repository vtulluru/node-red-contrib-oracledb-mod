module.exports = function (RED) {
  ...
  const thickInitState: { initialized: boolean; libDir: string; configDir: string } = {
    initialized: false,
    libDir: "",
    configDir: ""
  };
  ...
  function OracleServer(n) {
    ...
    if (isNaN(node.stmtcachesize)) node.stmtcachesize = 30;
    ...
    if (isNaN(node.maxretries)) node.maxretries = 3;
    ...
    function samplePeaks() {
      ...
      if (inUse > node.peakConnectionsInUse) node.peakConnectionsInUse = inUse;
      try {
        const s = typeof node.pool.getStatistics === "function" ? node.pool.getStatistics() : null;
        const qLen = s ? s.currentQueueLength : 0;
        if (qLen > node.peakQueueLength) node.peakQueueLength = qLen;
      } catch { /* ignore */ }
    }
    ...
    async function connect() {
      ...
      if (node.usethickmode) {
        try {
          if (!thickInitState.initialized) {
            const initOpts: any = {};
            if (node.instantclientpath) initOpts.libDir = node.instantclientpath;
            if (node.configdir) initOpts.configDir = node.configdir;
      ...
      if (!node.usethickmode) {
        if (node.configdir) poolConfig.configDir = node.configdir;
        if (node.walletlocation) poolConfig.walletLocation = node.walletlocation;
        if (node.walletpassword) poolConfig.walletPassword = node.walletpassword;
      }
    ...
    async function withRetry<T>(op: () => Promise<T>, label: string, requestingNode: any): Promise<T> {
      let lastErr: any;
      for (let attempt = 0; attempt <= node.maxretries; attempt++) {
        try {
          ...
          lastErr = err;
          if (!isTransientError(err) || attempt === node.maxretries) throw err;
    ...
    function scheduleStatusReset(rn: any, delayMs: number) {
      if (rn._oracleStatusTimer) clearTimeout(rn._oracleStatusTimer);
      rn._oracleStatusTimer = setTimeout(() => {
        rn._oracleStatusTimer = null;
        if (node.pool) rn.status({ fill: "green", shape: "dot", text: "connected" });
      }, delayMs);
    }
  ...
  RED.httpAdmin.post("/oracle-server/test", RED.auth.needsPermission("flows.write"), async function (req, res) {
    ...
    if (body.nodeId) {
      ...
      if (!user) user = cred.user;
      if (!password || password === "__PWRD__") password = cred.password;
      if (!walletPassword || walletPassword === "__PWRD__") walletPassword = cred.walletpassword;
    }
    const useThick = body.usethickmode === true || body.usethickmode === "true";
    if (useThick && !thickInitState.initialized) {
      try {
        const opts: any = {};
        if (body.instantclientpath) opts.libDir = body.instantclientpath;
        if (body.configdir) opts.configDir = body.configdir;
    ...
    const cfg: any = { connectString, user, password };
    if (!useThick) {
      if (body.configdir) cfg.configDir = body.configdir;
      if (body.walletlocation) cfg.walletLocation = body.walletlocation;
      if (walletPassword) cfg.walletPassword = walletPassword;
    }
    let conn;
    const t0 = Date.now();
    let tConnect = 0;
    try {
      ...
      try {
        const s = await conn.execute(
          ...
          [], { outFormat: oracledb.OUT_FORMAT_ARRAY, maxRows: 200 }
        );
        schemas = (s.rows as any[]).map(row => row[0]);
  ...
  RED.httpAdmin.get("/oracle-server/tnsnames", RED.auth.needsPermission("flows.write"), function (req, res) {
    ...
    if (typeof req.query.dir === "string" && req.query.dir.trim()) candidates.push(req.query.dir.trim());
    if (typeof req.query.walletLocation === "string" && req.query.walletLocation.trim()) candidates.push(req.query.walletLocation.trim());
    if (process.env.TNS_ADMIN) candidates.push(process.env.TNS_ADMIN);
    // De-dupe while preserving order.
    const seen = new Set<string>();
    const ordered = candidates.filter(c => { const r = path.resolve(c); if (seen.has(r)) return false; seen.add(r); return true; });

    function tryNext(i: number, tried: string[]) {
      if (i >= ordered.length) {
      ...
      fs.readFile(tnsPath, "utf8", (err, data) => {
        if (err) return tryNext(i + 1, tried);
        const aliases: string[] = [];
        const re = /^([A-Za-z0-9_$#.]+)\s*=/gm;
        let m;
        while ((m = re.exec(data)) !== null) aliases.push(m[1]);
