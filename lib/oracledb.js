module.exports = function (RED) {
    ...
    function initialize(node) {
        node.debug("node.server:"+node.server);
        if (node.server) {
    ...
    function OracleServer(n) {
        ...
        node.log("Oracle user:" + node.user);
        node.log("Oracle host:" + node.user);
        node.log("Oracle port:" + node.port);
        node.log("Oracle database:" + node.db);
        ...
        node.claimConnection = function (requestingNode, msg) {
            ...
            if (typeof node.credentials.user == 'undefined') {
                if (node.firstConnection = true) {
                    node.status.emit("error", "Oracle user credentials not set in the configuration. Please set and try again.");
                    let errorCode = err.message.slice(0, 9);
                    requestingNode.error("User credentials not set in the configuration. Please set and try again.");
