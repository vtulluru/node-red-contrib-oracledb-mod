...
module.exports = function (RED) {
    ...
    function OracleServer(n) {
        ...
        node.query = async function (msg, requestingNode, query, bindVars, resultAction, resultSetLimit, useMany, txAction) {
            function sendSuccess(msgToSend) {
            ...
            function sendError(errMsg, msgToSend) {
            ...
            try {
                ...
                if (Array.isArray(query)) {
                    const queryList = query.filter((q) => typeof q === "string" && q.trim().length > 0);
                    arrayResults = [];
                    for (let i = 0; i < queryList.length; i++) {
                        ...
                        const singleRes = await withRetry(() => connection.execute(singleQ, bindVars || [], options), `execute[${i}]`, requestingNode);
                        ...
                        if (typeof singleRes.rowsAffected === "number")
                            totalRowsAffected += singleRes.rowsAffected;
                    }
                }
                ...
                if (arrayResults) {
                    ...
                    if (totalRowsAffected > 0)
                        stats.rowsAffected = totalRowsAffected;
                }
                else if (result && typeof result.rowsAffected === "number") {
